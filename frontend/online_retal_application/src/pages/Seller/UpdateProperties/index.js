...
function UpdateProperty() {
  ...
  useEffect(() => {
    if(state?.value?.porperty_image)
    {
      setSelectedImages(state?.value?.porperty_image)
    }
  }, [state?.value?.porperty_image])
  ...
  return (
    <Formik
    initialValues={{
      porperty_name:state?.value?.porperty_name || "",
      porperty_description: state?.value?.porperty_description || "",
      porperty_price: state?.value?.porperty_price || "",
      porperty_image: state?.value?.porperty_image || [],
      porperty_Type:  state?.value?.porperty_Type || "",
      Address: state?.value?.Address || "",
      city: state?.value?.city || "",
      nearby_landmark:  state?.value?.nearby_landmark || "",
    ...
    onSubmit={async (values) => {
      ...
      try {
        const response = await axios.put(`http://localhost:5001/api/updateproperty/${state?.value?._id}`, DATA, {
          headers: {
